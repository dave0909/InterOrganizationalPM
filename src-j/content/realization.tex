...
(RSA)~\citep{DBLP:journals/cacm/RivestSA83} scheme for attestation (only the owner of the private key can sign messages that are verifiable via the public key) and secure message encryption (only the owner of the private key can decode messages that are encrypted with the corresponding public key). \NewJ{The private key associated with the \Compo{TEE}'s hardware remains inaccessible, even to users possessing administrative privileges on the \Compo{Miner Node}.}
...
\begin{newj}
Our \Compo{Secure Miner}'s implementation takes input from local users via console application (\Compo{TEE Interface in \cref{sec:deployment}}). We store the descriptive information of the \Compo{Trusted App} (e.g., heap size, embedded files, and environment variables) in a JSON file. The EGo framework automatically generates an RSA key couple derived from the underlying \Compo{Intel SGX} key materials, that we adoperate for attestation purposes. We store this key couple in a PEM file, inside the \Compo{TEE}. In our implementation, the \Compo{Secure Miner} app keeps the references of the provisioner's \Compo{Log Server} in a dedicated JSON file alongside the label of the {\CId} attribute in their respective log partitions. We assume this configuration file to be embedded in the \Compo{TEE} at deployment time. We resort to a TLS~\citep{Thomas/2000:SSL-TLS} communication channel between miners and provisioners over the HTTP web protocol to secure the information exchange. During the functioning of the CONFINE protocol, the \Compo{Secure Miner} generates JSON data structures to keep track of cases received by provisioners' case responses. Upon successful execution, our \textit{HeuristicsMiner} implementation generates workflow nets corresponding to the analyzed processes. Our implementation of CONFINE, including the \textit{HeuristicsMiner} in Go, is openly accessible at the following URL: \href{https://github.com/Process-in-Chains/CONFINE/}{\nolinkurl{github.com/Process-in-Chains/CONFINE/}}.    
