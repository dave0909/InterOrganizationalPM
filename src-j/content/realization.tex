...
the measurement of the \Compo{Trusted App}% which is defined as the hash value of the combination of its source code and data.
...
The \Compo{Log Server}s decrypt the received report and verifies that it has been signed by a functioning \Compo{TEE} (4). Typically, this step involve the CPU manifacturer that validates the report using the public key paired with the \Compo{Attestation Key} embedded within the processor. Then, the \Compo{Log Server}s undertake a comparison procedure in which they juxtapose the measurement found within the decrypted report against a predefined reference value associated with the source code of the \Compo{Secure Miner} (5). If the measurement matches the predefined value and the identity evidence authenticates the sender as a rightful miner organization (6), the \Compo{Miner Node} gains trust from the provisioner.
\end{newj}
