...
\item[\NewJ{provisioner}] \NewJ{if it delivers local event logs to be collaboratively mined, according to \cref{def:provisioner}};
...
\item[\Compo{Log Recorder}] registers the events taking place in the organizations' systems \NewJ{and builds the provisioner's log partition, introduced by \cref{def:partition}}.
