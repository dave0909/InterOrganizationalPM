...
In \cref{tab:trace}, we show the cases 312 and 711 and the corresponding traces recorded by the \Actor{Hospital} (i.e., $T_{312}$ and $T_{711}$). The \Actor{Specialized clinic} %(i.e., $T^S_{312}$ and $T^S_{711}$), 
and the \Actor{Pharmaceutical company} %(i.e., $T^C_{312}$ and $T^C_{711}$).
have their internal ids. However, interactions between the \Actor{Hospital}, and the \Actor{Pharmaceutical company} and the \Actor{Specialized clinic} always have references to the \Actor{Hospital} case id of the patients involved. In \cref{Tab:tab1} you can see references to \Actor{Hospital} ids in the first column. Since this link is present, it is not necessary that the case ids of \Actor{Pharmaceutical company} and \Actor{Specialized clinic} are synchronized with the hospital cases id.
