...
\KwIn{%
	$\LPrvS \ni \{ \LPrv_1, \ldots, \LPrv_n \}$, the (references to) $n$ log provisioners;%
	\newline
...
\KwData{%
	...
	$\CStor : \CIdU \to \CasU$, a map from case references $\CId \in \CIdU$ to a set of case parts in $\CasU$
}
%\KwOut{%
...
\UponEv(\tcp*[f]{The \Compo{Log Provider} gets the case request (line~\ref{alg:CasesReq:call})}){\upshape$\langle \LPrv.\Compo{LogProvider}, \Mesg{CasesReq} \:|\: \SecM, \SegSize, \CIdS \rangle$ \label{alg:CasesReq}}{%
	\If(\tcp*[f]{The \textit{attestation phase} (see \cref{fig:attestation})}){\upshape \SecM.\Compo{LogReceiver}.\Call{getAttestationReport}(\LPrv) is valid}{
		$\{\Segm_1, \ldots, \Segm_m\} \gets \Call{segmentEventLog}( \Compo{LogRecorder}.\Call{accessEventLog}(\CIdS), \SegSize )$\\
		\ForEach{$i \in \{1, \ldots, m\}$}{%
			\Trigger{\upshape $\langle \SecM.\Compo{LogReceiver}, \Mesg{CasesRes} \:|\: \LPrv, \Segm_i \rangle$ }
		}
	}
}
\UponEv{\upshape $\langle \SecM.\Compo{LogReceiver}, \Mesg{CasesRes} \:|\: \LPrv, \Segm \rangle$ \SuchThat{$\LPrv \in \LPrvS$}}{
	% See {\LaTeX} comments\\
	...
	\ForEach{\upshape $\CasP \in \Segm$}{%
		\If{\upshape $\Call{\CId}(\CasP) \in \LPrvMap[\LPrv]$}{%
			$\LPrvMap[\LPrv] \gets \LPrvMap[\LPrv] \,\setminus\, \{\Call{\CId}(\CasP)\}$ \;
			$\CIDMap[\Call{\CId}(\CasP)] \gets \CIDMap[\Call{\CId}(\CasP)] \,\setminus\, \{\LPrv\}$ \;
			$\Compo{LogManager}.\Call{mergeAndStore}(\CStor, {\CId}(\CasP), \CasP)$
		}
	}
}
...
\Upon{\upshape $\CIDMap[\CId]= \emptyset$ for some $\CId \in \textrm{dom}(\CIDMap)$} {%
	...
	\Yield{\upshape \CStor[\CId] \To{}} \Compo{LogElaborator}
}
