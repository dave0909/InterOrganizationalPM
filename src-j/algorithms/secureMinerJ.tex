...
\UponEv(\tcp*[f]{The \textit{initialization} phase begins here -- \cref{fig:init}}){\upshape $\langle \SecM.\Compo{LogRequester}, \Mesg{Init} \:|\: \LPrvS, \SegSize \rangle$}{
	\ForEach(\tcp*[f]{Request the list of cases provided by every \Compo{Log Provider} $\LPrv$}){$\LPrv \in \LPrvS$}{%
		\Trigger{\upshape $\langle \LPrv.\Compo{LogProvider}, \Mesg{CaseRefsReq} \:|\: \SecM \rangle$\label{alg:CaseRefsReq:call}} \tcp*{$\LPrv$ will reply with its case references (see \hyperref[alg2:CasesRefReq]{Alg.2 line 1})}
	}
	\Upon(\tcp*[f]{Once all $\LPrv$s have answered {\SecM} requests their cases}){\upshape $|\LPrvS| = |\textrm{dom}(\LPrvMap)|$}{%
		...
		{$\LPrv \in \LPrvS$}{
			...
			\tcp*[f]{see \hyperref[alg2:CasesReq]{Alg.2 line 5}}
...
\UponEv(\tcp*[f]{{\SecM} gets case references (see \hyperref[alg2:sendRefs]{Alg.2 line 4})}){\upshape $\langle \SecM.\Compo{LogRequester}, \Mesg{CaseRefsRes} \:|\: \LPrv, \CIdS \rangle$ \SuchThat{$\LPrv \in \LPrvS$} \label{alg:CaseRefsRes}}{
